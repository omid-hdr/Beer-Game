...
def get_report_handlers(repo: IGameRepository):
    """Factory function to inject the repository into the handler."""
    ...
    async def generate_reports(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Now repo is perfectly accessible from the outer scope!
        ...
        await context.bot.send_photo(
            ...
            caption=f"📈 **Global Total Costs - Game {game_id}**",
            parse_mode="Markdown"
        )

        # 2. Team-specific charts (Sending as a media group to prevent spam)
        for team_code in game.teams.keys():
            inv_buf = create_team_inventory_chart(game, team_code)
            # You would also generate the cost_buf here
